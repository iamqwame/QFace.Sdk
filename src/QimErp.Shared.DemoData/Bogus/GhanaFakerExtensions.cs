...
public static class GhanaFakerExtensions
{
    ...
    public static string GhanaFirstName(this Faker f, Gender gender) =>
        gender == Gender.Male
            ? f.Random.ListItem(GhanaPersonNames.MaleFirstNames.AsList())
            : f.Random.ListItem(GhanaPersonNames.FemaleFirstNames.AsList());

    public static string GhanaMiddleName(this Faker f, Gender gender) =>
        gender == Gender.Male
            ? f.Random.ListItem(GhanaPersonNames.MaleMiddleNames.AsList())
            : f.Random.ListItem(GhanaPersonNames.FemaleMiddleNames.AsList());

    public static string GhanaLastName(this Faker f) =>
        f.Random.ListItem(GhanaPersonNames.LastNames.AsList());
    ...
    public static string GhanaPhone(this Faker f, GhanaData.GhanaTelecom.Carrier? carrier = null)
    {
        for (var attempt = 0; attempt < 8; attempt++)
        {
            ...
            var prefixes = GhanaTelecom.PrefixesFor(resolvedCarrier);
    ...
    private static GhanaData.GhanaTelecom.Carrier PickCarrier(Faker f)
    {
        ...
        return roll switch
        {
            < 0.60 => GhanaTelecom.Carrier.MTN,
            < 0.80 => GhanaTelecom.Carrier.Telecel,
            _ => GhanaTelecom.Carrier.AT
        };
    }

    public static string GhanaRegion(this Faker f) =>
        f.Random.ListItem(GhanaGeography.Regions.AsList());

    public static string GhanaCity(this Faker f, string? region = null)
    {
        if (region != null && GhanaGeography.CitiesByRegion.TryGetValue(region, out var cities))
            return f.Random.ListItem(cities.AsList());
        var pickedRegion = f.Random.ListItem(GhanaGeography.CitiesByRegion.Keys.ToList());
        return f.Random.ListItem(GhanaGeography.CitiesByRegion[pickedRegion].AsList());
    }

    public static string GhanaStreet(this Faker f) =>
        $"{f.Random.ListItem(GhanaGeography.Streets.AsList())} {f.Random.Number(1, 9999)}";

    public static string GhanaGpsCode(this Faker f, string? region = null)
    {
        var resolved = region ?? f.Random.ListItem(GhanaGeography.Regions.AsList());
        var prefix = GhanaGeography.RegionGpsPrefix.TryGetValue(resolved, out var p) ? p : "GA";
        return $"{prefix}-{f.Random.Number(100, 999)}-{f.Random.Number(1000, 9999)}";
    }

    public static string GhanaCard(this Faker f) =>
        f.Random.Replace(GhanaIdentification.GhanaCardPattern);
    ...
    public static string GhanaTin(this Faker f) =>
        f.Random.Replace(GhanaIdentification.TinPattern);

    public static string GhanaPassport(this Faker f) =>
        f.Random.Replace(GhanaIdentification.PassportPattern);
    ...
    public static string GhanaEthnicGroup(this Faker f) =>
        PickWeighted(f, GhanaEthnicity.Distribution);

    public static string GhanaReligionLabel(this Faker f) =>
        PickWeighted(f, GhanaReligion.Distribution);

    public static string GhanaDenominationFor(this Faker f, string religion) => religion switch
    {
        "Christian" => PickWeighted(f, GhanaReligion.ChristianDenominations),
        "Islam" => PickWeighted(f, GhanaReligion.MuslimDenominations),
        _ => string.Empty
    };

    public static GhanaData.GhanaBanks.Bank GhanaBank(this Faker f) =>
        f.Random.ListItem(GhanaBanks.CommercialBanks.AsList());
